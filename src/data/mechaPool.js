...
export function generateMechaName(usedNames) {
  for (let i = 0; i < 100; i++) {
...
export function generateMecha(usedNames) {
  const name = generateMechaName(usedNames);
  const descriptorCount = 3 + Math.floor(Math.random() * 4); // 3-6
